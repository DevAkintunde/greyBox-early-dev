...
import PText from "../../_cms/models/entities/paragraphs/PText.model.js";
import PImage from "../../_cms/models/entities/paragraphs/PImage.model.js";
import PVideo from "../../_cms/models/entities/paragraphs/PVideo.model.js";
import {
  BAD_REQUEST,
  SERVER_ERROR,
  OK,
...
import Video from "../../_cms/models/entities/media/Video.model.js";
    ...
    if (modelAttributes.body) {
      if (Object.keys(modelAttributes.body).length > 0) {
        ...
        let updateNoded = await sequelize.transaction(async (t) => {
          let node = await Service.findOne(
            {
              where: {
...
export const updateAlias = async (ctx, next) => {
  try {
    let searchKey = ctx.request.body.uuid ? "uuid" : "currentAlias";
    let thisNode = await Service.findOne({
      where: {
        [searchKey === "currentAlias" ? "alias" : searchKey]:
          ctx.request.body[searchKey],
      },
    });
    if (thisNode instanceof Service) {
      thisNode.update({
      ...
      ctx.state.error = {
        status: NOT_FOUND,
        statusText: `The service to be updated does not seem to exist`,
      };
    }
  } catch (err) {
    logger.error("Node Controller", err);
    ctx.state.error = {
      status: SERVER_ERROR,
      statusText: `Unable to update service alias`,
    };
  }
  await next();
};
...
export const updateStatus = async (ctx, next) => {
  try {
    ...
    ctx.state.error = {
      status: SERVER_ERROR,
      statusText: `Unable to update service status`,
    };
  }
  await next();
};
...
export const deleteItem = async (ctx, next) => {
  try {
    let deletionCall = await Service.destroy({
      where: ctx.request.body,
    });
    if (!deletionCall) {
      ctx.state.error = {
        status: SERVICE_UNAVAILABLE,
        statusText: `Unable to delete service node`,
      };
    }
  } catch (err) {
...
export const viewItem = async (ctx, next) => {
  try {
    let node = await sequelize.transaction(async (t) => {
      const thisNode = await Service.findOne(
        { where: ctx.request.body },
        { transaction: t }
      );
