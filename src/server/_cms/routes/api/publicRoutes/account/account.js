  ...
  async (ctx) => {
    ...
    return passport.authenticate("local", async (err, user, info) => {
      ...
      if (!user) {
        ...
        let thiUserAccessTimestamps = await sequelize.transaction(async (t) => {
          let userAccessTimestamps = await UserAccessTimestamps.findByPk(
            user.dataValues.uuid,
            { transaction: t }
          );
          if (userAccessTimestamps instanceof UserAccessTimestamps) {
            await userAccessTimestamps.update(
              { signedIn: Date.now() },
              { transaction: t }
            );
            return userAccessTimestamps.toJSON();
          } else {
            userAccessTimestamps = await UserAccessTimestamps.create(
              {
                account_id: user.dataValues.uuid,
                signedIn: Date.now(),
              },
              { transaction: t }
            );
        ...
        let profileData = {
          ...
          profile: {
            ...user.toJSON(),
            type: "Admin",
            access: thiUserAccessTimestamps,
          },
        };
