...
export const TabMenu = ({ entity }: any) => {
  ...
  useEffect(() => {
    let isMounted = true;
    let accessTab = false;
    //tab access instances
    let authPathCheck = location.pathname.split("/auth/")[1];
    if (authPathCheck) {
      let pathParams = authPathCheck.split("/");
      if (pathParams[0] === "media" && pathParams.length === 3) {
        accessTab = true;
      } else if (pathParams[0] !== "media" && pathParams.length === 2) {
        accessTab = true;
      } else if (
